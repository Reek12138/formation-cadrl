...
class CustomEnv:
    ...
    def __init__(self, width=100, height=100, num_obstacles=15, agent_radius=1, obs_radius = 2,safe_theta = 8,target_radius = 4,
        ...
        self.SAC1 = SAC(state_dim = (4+2+3*5+2+5*(self.follower_uav_num-1) + 2 ),
                        #    state_dim = (4+4+5*self.num_obstacles+2+5*(self.follower_uav_num-1))
                                                            hidden_dim = 512,
                                                            action_dim=2,
                                                            actor_lr=1e-4,
        ...
        self.SAC2 = SAC(state_dim = (4+2+3*5+2+5*(self.follower_uav_num-1) + 2 ),
                        #    state_dim = (4+4+5*self.num_obstacles+2+5*(self.follower_uav_num-1))
                                                            hidden_dim = 512,
                                                            action_dim=2,
                                                            actor_lr=1e-4,
                                                            ...
                                                            device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"),
                                                            agent_num=self.follower_uav_num)
        self.SAC3 = SAC(state_dim = (4+2+3*5+2+5*(self.follower_uav_num-1) + 2 ),
                        #    state_dim = (4+4+5*self.num_obstacles+2+5*(self.follower_uav_num-1))
                                                            hidden_dim = 512,
                                                            action_dim=2,
                                                            actor_lr=1e-4,
    ...
    def observe(self):
        ...
        for obs_id, obs in sorted_obs[:6]:
            ...
            if _obs_distance - self.agent_radius - self.obs_radius <= self.obs_delta*0.5:
                obs_num += 1
        ...
        for i in range (self.follower_uav_num):
            ...
            # for obs_id, obs in self.obstacles.items():
                ...
                # obs_distance_angle.extend([obs_distance, obs_angle])
                _obs_distance = _obs_distance - self.obs_radius - self.agent_radius
                if _obs_distance - self.obs_radius - self.agent_radius <= self.obs_delta*0.2:
                    obs_num_follower += 1
    ...
    def _apply_follower_actions(self, follower_actions):
        for i in range (self.follower_uav_num):
            self.follower_uavs[f"follower_{i}"].vel[0] = follower_actions[2*i] *2.0
            self.follower_uavs[f"follower_{i}"].vel[1] = follower_actions[2*i + 1] *2.0
    ...
    def _apply_leader_action(self, action):
        ...
        linear_vel = (action[0] + 1)*0.6 # 线速度，带偏置
...
# ======计算领航者的奖励====================
    def _calculate_leader_reward(self, leader_action, last_distance, last_obs_distance):
        ...
        # print(f"leader target_reward = {reward2}, obs_reward = {reward3}, vel_reward = {reward4}, side_reward = {reward5}")
...
# ======计算跟随者的奖励===================
        ...
        for obs_id, obs in self.obstacles.items():
            ...
            # if dis <= self.obs_delta*1.0 :     
                ...
                vo_flag, min_exp_time, min_dis = self.rvo_inter.config_vo_reward(robot_state=robot_state,
                                                                                                                                                        ...
                                                                                                                                                        obs_line_list=obs_line_list,
                                                                                                                                                        action=action)
                if vo_flag and dis <= self.obs_delta - 2 - self.obs_radius - self.agent_radius:
    ...
    def render(self, display_time = 0.1):
        ...
        # ====== 在这里，额外画出三角形 ======
        # 1. 收集三个跟随者的位置
        if self.follower_uav_num == 3:
            follower_positions = [
                self.follower_uavs["follower_0"].pos,
                self.follower_uavs["follower_1"].pos,
                self.follower_uavs["follower_2"].pos
            ]
            
            # 2. 分别画三条线段把它们连起来 (0->1, 1->2, 2->0)
            #   或者用一个多边形(MPolygon)来画闭合图形

            # 方式A：用三条 plot 画线
            for i in range(3):
                j = (i + 1) % 3
                x_coords = [follower_positions[i][0], follower_positions[j][0]]
                y_coords = [follower_positions[i][1], follower_positions[j][1]]
                self.ax.plot(x_coords, y_coords, color='black', linewidth=1.5)

                # from matplotlib.patches import Polygon
                # tri = Polygon(follower_positions, closed=True, fill=False, edgecolor='black', linewidth=1)
                # self.ax.add_patch(tri)
            # === 在这里，额外增加“领航者与每个跟随者之间的连线” ===
            line_colors = ['red', 'green', 'blue']  # 3 条线的颜色

            leader_pos = self.leader_agent.pos
            for i in range(self.follower_uav_num):
                # 获取第 i 个跟随者的位置
                follower_pos = self.follower_uavs[f"follower_{i}"].pos
                
                # 画领航者到跟随者的连线（以 i 为索引来选颜色）
                self.ax.plot(
                    [leader_pos[0], follower_pos[0]],
                    [leader_pos[1], follower_pos[1]],
                    color=line_colors[i % len(line_colors)],  # 取第 i 个颜色
                    linewidth=1.5
                )
