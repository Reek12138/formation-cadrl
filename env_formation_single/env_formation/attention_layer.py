...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, temp_factor=0.5, dropout=0.1):
        super().__init__()
        ...
        # 调整temperature: 原来是 d_k**0.5，这里可以再除以一个系数
        self.attention = ScaledDotProductAttention(temperature=(d_k**0.5)/temp_factor, dropout=dropout)
    ...
    def forward(self, q, k, v, mask=None):
        ...
        # 2) 交换维度，使得 (batch_size, n_head, seq_len, d_k/d_v)
        q, k, v = q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2)

        # 3) 计算 Scaled Dot-Product Attention
        q, attn = self.attention(q, k, v, mask=mask)

        # 4) 变回 (batch_size, seq_len, n_head*d_v)
        q = q.transpose(1, 2).contiguous().view(size_b, len_q, -1)
...
class ScaledDotProductAttention(nn.Module):
    def __init__(self, temperature, dropout=0.1):
        super().__init__()
        self.temperature = temperature
        self.dropout = nn.Dropout(p=dropout)  # 这里实例化Dropout
    ...
    def forward(self, q, k, v, mask=None):
        ...
        attn = F.softmax(attn, dim=-1)
        attn = self.dropout(attn)  # 在注意力分布上做dropout
        output = torch.matmul(attn, v)
        return output, attn
