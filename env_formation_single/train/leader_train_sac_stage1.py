...
from env_formation_single.env_formation.env_formation_single_stage1 import CustomEnv
...
for episode_i in range(NUM_EPISODE):
    ...
    if episode_i <= 5000:
        NOISE = 0.2
    elif episode_i > 5000 and episode_i <= 10000:
        NOISE = 0.2
    else:
        NOISE = 0.1
    ...
    for step_i in range(NUM_STEP):
        ...
        noise = np.random.normal(0, NOISE, size=leader_action.shape)
...
#====== 每个episode结束后更新一次=====================================================================
    # print("buffer size", env.leader_agent.replay_buffer.size() )
    ...
    # if episode_i > NUM_EPISODE/3 and episode_i % RENDER_FREQUENCE == 0:
        ...
        for test_episode_i in range (RENDER_NUM_EPISODE):
            ...
            for test_step_i in range (RENDER_NUM_STEP):
                ...
                leader_action = env.leader_agent.sac_network.take_action(leader_state)
                noise = np.random.normal(0, 0.01, size=leader_action.shape)
                noisy_action = leader_action + noise
                noisy_action = np.clip(noisy_action, -1, 1)
                action_in =[noisy_action[0] + 1,
                            noisy_action[1] * (np.pi/2)]
